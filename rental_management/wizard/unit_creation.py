    ...
    def action_create_property_unit(self):
        ...
        if self.total_floors <= 0:
            return {
                ...
                'params': {
                    'type': 'info',
                    'title': self.env._('Total Floor !'),
                    'message': self.env._("Total floor should be greater than 1."),
                    'sticky': False,
                }
            }
        if self.units_per_floor <= 0:
            return {
                ...
                'params': {
                    'type': 'info',
                    'title': self.env._('Total Unit Per Floor !'),
                    'message': self.env._('Total unit per floor should be greater than 1.'),
                    'sticky': False,
                }
            }
        if not active_id:
        ...
        property_static_rec = {
            ...
            'brocher_access_token': secrets.token_urlsafe(12)
        }
        ...
        for floor in range(self.floor_start_from, self.total_floors + self.floor_start_from):
            for unit in range(1, self.units_per_floor + 1):
                code = f"{self.property_code_prefix}{str(floor).zfill(2)}-{str(unit).zfill(2)}"
                name = f"{project_id.name}-{str(floor) + str(unit).zfill(2)}"
                floor_now = floor
        ...
        unit_amenities, unit_images, unit_specification, unit_connectivity = (
            self.get_property_availability(unit_from=unit_from, project_id=project_id))
